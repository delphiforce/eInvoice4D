...
class procedure TeiCoreValidator.Validate(const AInvoice: IFatturaElettronicaType; const AResult: IeiValidationResultCollection);
begin
  ValidateProp(AInvoice, AResult);
end;

class procedure TeiCoreValidator.ValidateProp(const AProp: IeiBaseProperty; const AResult: IeiValidationResultCollection);
var
  ...
  case AProp.Kind of
    pkValue:
      begin
        ...
        if (AProp.Occurrence = TeiOccurrence.o11) and AProp.IsNull then
          AResult.Add(TeiValidatorsFactory.NewValidationResult(AProp.FullQualifiedName, String.Empty, 'Proprietà obbligatoria non valorizzata', vkCore));
        // MinLength - MaxLength
    ...
    pkBlock:
      begin
        // Occurrence
        if (AProp.Occurrence = TeiOccurrence.o11) and AProp.IsNull then
          AResult.Add(TeiValidatorsFactory.NewValidationResult(AProp.FullQualifiedName, String.Empty, 'Proprietà/blocco obbligatorio non presente', vkCore));
        // Recursion
    ...
    pkList:
      begin
        ...
        if (AProp.Occurrence = TeiOccurrence.o1N) and AProp.IsEmptyOrZero then
          AResult.Add(TeiValidatorsFactory.NewValidationResult(AProp.FullQualifiedName, String.Empty, 'La lista deve contenere almeno un elemento', vkCore));
        // Recursion
        for LProp in (AProp as IeiComplexProperty) do
          ValidateProp(LProp, AResult);
      end;
